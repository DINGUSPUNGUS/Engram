...
``docker/compose.yaml`` (the repo's one, canonical compose file — see its own
header comment); these are plain text checks, not full YAML/Dockerfile
...
def test_compose_publishes_ports_loopback_only() -> None:
    ...
    for port in ("8000", "3000"):
        ...
        assert mapping in compose, (
            f"docker/compose.yaml must publish port {port} as {mapping} — a bare"
