...
| Surface | Risk | Stance |
| --- | --- | --- |
| API port | Anything reaching it can read/write all memories | Binds `127.0.0.1` by default. Do not expose beyond loopback before the auth milestone. Changing the bind host is an explicit, documented opt-in. In Docker the container's own process binds `0.0.0.0` (a Docker networking requirement — a container-internal `127.0.0.1` bind is unreachable via `-p` from anywhere), so the actual loopback control point is the host-side publish mapping; `docker/compose.yaml` publishes `127.0.0.1:8000:8000` for exactly this reason — widening it is the explicit opt-in. |
